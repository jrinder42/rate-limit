    ...
    def decorator(func):
        def wrapper(user_id, *args, **kwargs):
            if user_id not in buckets:
                buckets[user_id] = SyncLeakyBucket(LeakyBucketConfig(capacity=capacity, seconds=seconds))
...
def async_rate_limit_per_user_v2(capacity=10, seconds=1, max_users=1000, ttl=None):
    ...
    def decorator(func):
        async def wrapper(user_id, *args, **kwargs):
            if user_id not in buckets:
                buckets[user_id] = AsyncLeakyBucket(LeakyBucketConfig(capacity=capacity, seconds=seconds))
            bucket = buckets[user_id]
            async with bucket:
...
if __name__ == "__main__":
    ...
    def something_user(uid):  # first parameter is the user_id
        print(f"User {uid} called at {time.strftime('%X')}")

    for _ in range(10):  # everything should pass through immediately
        try:
            something_user(_)
        except Exception as e:
            print(f"Rate limit exceeded: {e}")
